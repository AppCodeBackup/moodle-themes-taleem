...
function theme_taleem_process_css($css, $theme) {
    ...
    $css = theme_taleem_pre_css_set_fontwww($css);
 ...
 * @return bool
 */
function theme_taleem_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    static $theme;
 ...
 *
 */
function theme_taleem_send_unmodified($lastmodified, $etag) {
 ...
 */
function theme_taleem_set_fontwww() {
    global $CFG, $PAGE;

    $themewww = $CFG->wwwroot."/theme";
    $theme = theme_config::load('taleem');
    $fontwww = '$fontwww: "'. $themewww.'/taleem/fonts/"'.";\n";
    return $fontwww;
}
 ...
 * @return string
 */
function theme_taleem_pre_css_set_fontwww($css) {
    global $CFG, $PAGE;

    $themewww = $CFG->wwwroot."/theme";
    $tag = '[[setting:fontwww]]';
    $theme = theme_config::load('taleem');
    $css = str_replace($tag, $themewww.'/taleem/fonts/', $css);
    return $css;
}
 ...
 * @return bool
 */
function theme_taleem_get_setting($setting, $format = false) {
    global $CFG;
 ...
 * @return string
 */
function theme_taleem_theme_url() {
    global $CFG, $PAGE;
 ...
 * @return array
 */
function theme_taleem_hidden_courses_ids() {
    global $DB;
    $hcourseids = array();
 ...
 * @return string
 */
function theme_taleem_strip_html_tags( $text ) {
    $text = preg_replace(
        array(
 ...
 * @return string $out
 */
function theme_taleem_course_trim_char($str, $n = 500, $endchar = '&#8230;') {
    if (strlen($str) < $n) {
        return $str;
    }
 ...
 * @return string
 */
function theme_taleem_lang($key = '') {
    $pos = strpos($key, 'lang:');
    if ($pos !== false) {
        list($l, $k) = explode(":", $key);
        if (get_string_manager()->string_exists($k, 'theme_taleem')) {
            $v = get_string($k, 'theme_taleem');
            return $v;
        } else {
            return $key;
        }
