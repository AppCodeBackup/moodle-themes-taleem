 ...
 * @copyright   2018 Vowelweb Development Team, vowelweb.com
 * @author      Vowelweb Development Team
 ...
 * @copyright  2015 onwards Vowelweb Development Team (http://www.lmsace.com)
 * @author    Vowelweb Development Team
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class theme_taleem_core_course_renderer extends core_course_renderer {
    ...
    public function frontpage_available_courses() {
        ...
        if (count($rcourseids) > 0) {
            ...
            foreach ($rcourseids as $courseid) {
                ...
                $no = get_config('theme_taleem', 'patternselect');
                ...
                <a href="'.$courseurl.'"><img src="'.$imgurl.'" width="243" height="165" alt="'.$course->fullname.'"></a></div>
                <div class="fp-courseinfo"><h5>
    ...
    public function promoted_courses() {
        ...
        $pcourseenable = theme_taleem_get_setting('pcourseenable');
        if (!$pcourseenable) {
            return false;
        }
        ...
        $featuredids = theme_taleem_get_setting('promotedcourses');
        $rcourseids = (!empty($featuredids)) ? explode(",", $featuredids) : array();
        if (empty($rcourseids)) {
            return false;
        }

        $hcourseids = theme_taleem_hidden_courses_ids();
        ...
        if (!empty($fcourseids)) {
            foreach ($fcourseids as $courseids) {
                ...
                foreach ($courseids as $courseid) {
                    ...
                    $summary = theme_taleem_strip_html_tags($course->summary);
                    $summary = theme_taleem_course_trim_char($summary, 75);
                    ...
                    <div class="info"><h5><a href="'.$courseurl.'">'.$course->fullname.'</a></h5></div></div></div>';
    ...
    protected function coursecat_coursebox(coursecat_helper $chelper, $course, $additionalclasses = '') {
        ...
        if ($chelper->get_show_courses() < self::COURSECAT_SHOW_COURSES_EXPANDED) {
            if ($course->has_summary() || $course->has_course_contacts() || $course->has_course_overviewfiles()) {
                ...
                $image = $this->output->pixicon('i/info', $this->strings->summary);
